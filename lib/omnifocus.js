...
exports.addNewTask = osa((projectName, title, tag, typeTag, taskNote) => {
    ...
    var task = ofApp.Task({
        "name": title,
        "note": taskNote,
        // "tags": [tagFoundOrCreated(tag)],
        // "primaryTag": tagFoundOrCreated(tag)
    })
    // ofDoc.inboxTasks.push(task)
    project.tasks.push(task)
    ofApp.add(tagFoundOrCreated(tag), {
        to: task.tags
    })
    ofApp.add(tagFoundOrCreated(typeTag), {
        to: task.tags
    })
    return { "id": task.id(), "name": task.name() };
});
