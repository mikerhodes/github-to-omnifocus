...
async function syncNotifications(octokit, c) {
    try {
        ...
        for (var i = 0; i < notifications.data.length; i++) {
            ...
            var commentId = ""
            if (n.subject.latest_comment_url !== null) {
        ...
        const zipped = issues.map((issueResponse) => {
            return {
                ...
                prefix: prefixes.find(notification => {
                    ...
                    // associate properly.
                    // This is a little confusing, because the API URL uses the
                    // plural here but the HTML URL uses the singular.
        ...
        const potentialTasks = zipped.map(x => {
            ...
            potentialTask.body = x.issue.html_url
            if (x.prefix.commentId.length > 0) {
                potentialTask.body = potentialTask.body + "#issuecomment-" + x.prefix.commentId
            }
